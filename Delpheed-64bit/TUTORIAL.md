...
- Targets you unpack can be **32-bit or 64-bit**. Use `OEPScan` (32-bit) or `Delpheed`
  for 32-bit targets, and `OEPScan64` (64-bit) for 64-bit targets — the debugger has to
  match the target's bitness. The static class scanner (`VMTScan`) reads both.
...
**64-bit targets.** For a 64-bit packed binary, use `OEPScan64` instead — same options,
same output — then read the classes from the dump:

```
bin\OEPScan64.exe Packed64.exe Packed64_dump.exe /iat
bin\VMTScan.exe Packed64_dump.exe
```

`OEPScan64` uses the guard-page method (there's no ESP trick on x64) and is built
`dcc64`. `Unpack.exe` and the `Delpheed` one-shot are 32-bit only.
...
|---|---|
| `Not a 32-bit PE32 image` / `Not a 64-bit PE32+ image` | Wrong tool for the target's bitness — use `OEPScan` for 32-bit, `OEPScan64` for 64-bit. `VMTScan` reads either statically. |
...
| `CreateProcess failed` | Wrong path, or a bitness mismatch — build the 32-bit tools with `dcc32` and `OEPScan64` with `dcc64`, and match the tool to the target. |
