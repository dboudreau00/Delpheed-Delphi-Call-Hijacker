...
begin
  ...
  // 24 (file header) + 112 (to data dirs, PE32+) + (DDIR_IAT+1)*8 (through the IAT dir).
  if UInt64(lfanew) + 24 + 112 + (DDIR_IAT + 1) * 8 > UInt64(Length(f)) then
  begin
  ...
  // sizeOfHeaders drives the IAT-scan start and the section-room check; clamp it
  // so a bogus value cannot truncate to a negative scan index on a 32-bit build.
